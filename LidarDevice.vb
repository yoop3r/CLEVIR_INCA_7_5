...
Public Class LidarDevice
    ...
    Public Sub StartCapture(pcapFilename As String, sequence As Integer)
        ...
        Try
            ...
            ' ═══════════════════════════════════════════════════════════════════
            ' STEP 8: Register Packet Handler and arm the capturing flag BEFORE
            ' StartCapture() so OnPacketArrival never sees _isCapturing=False
            ' ═══════════════════════════════════════════════════════════════════
            _isCapturing = True
    ...
    ''' SharpPcap.PacketArrivalEventHandler signature: (sender As Object, e As PacketCapture)
    ''' </summary>
    Private Sub OnPacketArrival(sender As Object, e As Object)
        Try
            ' ✅ FIXED: Use reflection to access PacketCapture.GetPacket() (VB.NET can't use ref structs)
    ...
    Public Sub StopCapture()
        ...
        Try
            ...
            ' ═══════════════════════════════════════════════════════════════════
            ' Remove event handler (prevents memory leaks)
            ' ═══════════════════════════════════════════════════════════════════
            If _captureDevice IsNot Nothing Then
                RemoveHandler _captureDevice.OnPacketArrival, AddressOf OnPacketArrival
            End If
